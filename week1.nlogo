...
to-report agent-decision
  ...
  if (Agent-model = "ZI-C") [
    ...
    if (trader-type = "BUYER") [
      set price random (value - MaxPrice + 1) + MaxPrice
    ]
      ;Buyers should select random prices between their value and MaxPrice,
      ;ensuring they do not offer more than their perceived value of the commodity.
    if (trader-type = "SELLER") [
      set price random (MaxPrice - value + 1) + value
    ]
      ;Sellers should select random prices between their cost and MaxPrice,
      ;ensuring they do not sell below their cost.
  ]
  ]
  report price
end
  ...
  repeat 20 [
    ask turtles with [traded = false] [
      if (trader-type = "BUYER") [
        ...
        let candidate-seller min-one-of (other turtles with [trader-type = "SELLER" and offer-price >= 0 and traded = false]) [offer-price]
        if (candidate-seller != nobody and traded = false) [
          if (offer-price >= [offer-price] of candidate-seller) [
            set TransactionPrices lput ([offer-price] of candidate-seller) TransactionPrices
            set PricesAtTheRound lput ([offer-price] of candidate-seller) PricesAtTheRound
            set profit value - [offer-price] of candidate-seller
            set traded true
            ask candidate-seller [
              set profit offer-price - value
              set traded true
            ]
          ]
        ]
...
to draw-transaction-price
  ...
  clear-plot
  create-temporary-plot-pen "transaction price"
  ...
  create-temporary-plot-pen "equilibrium line"
...
to draw-demand-supply-functions
  set-current-plot "Demand and Supply Function"
  clear-plot let x 0
...
to draw-round-end-line
  set RoundLine lput (length TransactionPrices - 1) RoundLine
  set-current-plot "Transaction Price"
  foreach RoundLine [
    x ->
